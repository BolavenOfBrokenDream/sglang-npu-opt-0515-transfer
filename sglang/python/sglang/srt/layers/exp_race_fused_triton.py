# Fused exponential-race sampling consumption for Ascend NPU.
#
# Replaces the main-stream consumption chain
#   u.neg_().add_(1.0); torch.minimum(u, bound, out=u); u.log_().neg_()
#   sampled = torch.div(probs, u).argmax(dim=-1)
# with fused Triton kernels that read probs and u once and write one int32
# index per row.
#
...
# tile. Kernel 1 uses only compiler-verified idioms on triton-ascend:
#   - loop form `for v0 in tl.range(0, V, BLOCK_V)`, loop variable IS the
#     offset;
...
# BLOCK_V=4096 keeps the tile buffers within the per-AIV-core UB budget on
# 910C; 8192 overflows UB at compile time. If a future triton-ascend version
# still overflows, drop to 2048.
_BLOCK_V = 4096

# Partials reducer tile width: 64 covers any V <= 262144 with one tile.
_BLOCK_NT = 64
...
if _HAS_TRITON:
    ...
    def _exp_race_partial_argmax_kernel(
        ...
        # per-tile lane via tl.argmax (tie behavior unverified).
        row = tl.program_id(0).to(tl.int64)
        base = row * V
        pbase = row * NT
        lane = tl.arange(0, BLOCK_V)
...
def exp_race_argmax(probs: torch.Tensor, u: torch.Tensor) -> torch.Tensor:
    """Fused consumption: row argmax of probs / q(u), int32 [B].

    Two-kernel partials path (production, exact first-occurrence tie-break).
    Non-destructive: neither probs nor u is modified.
    """
